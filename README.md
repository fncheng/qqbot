...
- 支持在私聊或群内 `@机器人` 时读取被引用消息；文本进入会话上下文，图片在模型支持 Vision 时进入本次多模态请求。
...
机器人会通过 NapCat 回查被引用消息。旧消息或已撤回消息可能无法读取；图片识别要求当前模型和 OpenAI-compatible 服务商支持 Chat Completions 的 `image_url` 内容段。
...
私聊同样支持引用消息；只引用而不附加问题时，机器人会按“分析这条被引用的消息”处理。
