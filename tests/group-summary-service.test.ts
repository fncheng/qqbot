...
describe('群聊每日总结', () => {
  ...
  it('归档幂等，并按默认七天原始数据和缓存保留期发起清理', async () => {
    ...
    expect(repository.cleanups[0].archiveBefore.toISOString()).toBe('2026-09-11T01:00:00.000Z')
    expect(repository.cleanups[0].summaryBefore.toISOString()).toBe('2026-08-19T01:00:00.000Z')
  })
  ...
  it('只查询触发群在 Asia/Shanghai 当天的消息，并正确跨越当地午夜', async () => {
    ...
    expect(repository.listCalls[0].groupId).toBe('g1')
    expect(repository.listCalls[0].start.toISOString()).toBe('2026-09-17T16:00:00.000Z')
